...
func _on_Level_Pressed(m: int, button_id: TextureButton):
	...
	for button in range(level_buttons.size()):
		if button == Global.CURRENT_LEVEL:
...
func _on_Resolution_List_item_activated(index):
	...
	elif OS.get_screen_size() == RESOLUTIONS[index] or OS.get_screen_size().length() < RESOLUTIONS[index].length():
...
func set_res(x, y):
	...
	if full:
		_on_Full_Screen_toggled(true)
	elif OS.get_screen_size() == Vector2(x, y) or OS.get_screen_size().length() < Vector2(x, y).length():
		OS.window_borderless = true
		print("WOW")
	else:
