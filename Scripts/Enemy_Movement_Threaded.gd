...
func _physics_process(delta) -> void :
	...
	if move_speed == 0:
		if player_spotted and not dead and not tranq:
			...
			rotation.x = 0
		shoot_mode = true
	if not player_spotted and not dead and not tranq:
	...
	if Global.every_20 and player_distance < 30:
	...
	if not dead and not tranq:
		...
		if player_spotted:
			player_spotted()
		track_player(delta)
		if not sight_potential and player_distance > 20 and not Global.every_20 and not alerted and not player_spotted:
			return
...
func track_player(delta) -> void :
	...
	player_ray.look_at(player.global_transform.origin + player_offset, Vector3.UP)
...
func find_path(delta) -> void :
	...
	if anim_player.current_animation != "Attack":
			...
			footstep_counter += get_physics_process_delta_time()
